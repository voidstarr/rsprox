package net.rsprox.protocol.v233.game.incoming.encoder.codec.misc.client
    ...
    override fun encode(
        ...
        for (result in message.result) {
            if (result is ReflectionCheckReply.ErrorResult<*, *>) {
                val opcode = when(val type = result.throwable) {
...
public class SendPingReplyEncoder : ProxyMessageEncoder<SendPingReply> {
    ...
    override fun encode(
        ...
        buf.p4Alt3(message.value1)
        buf.p4(message.value2)
        buf.p1Alt2(message.gcPercentTime)
        return buf.buffer
    }
}
